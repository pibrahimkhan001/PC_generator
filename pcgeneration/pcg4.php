...
if(!empty($_POST['htno']) && !empty($_POST['stname']) && isset($_POST['aadhar']) && !empty($_POST['image']) && !empty($_POST['fname']) && !empty($_POST['percentage']) && !empty($_POST['mname']) && !empty($_POST['course']) && !empty($_POST['spec']) && !empty($_POST['gender']) && !empty($_POST['month']) && !empty($_POST['year']) && !empty($_POST['pcissued']) && !empty($_POST['class']) ){
$res=$_POST;
require('pcgs.php');
$a=new PCGS();
$cr=substr($_POST['htno'],5,1);
$sp=substr($_POST['htno'],6,2);
$x=$a->add_details($res['htno'],$res['stname'],$res['aadhar'],$res['image'],$res['fname'],$res['mname'],$res['gender'],$cr,$sp,$res['percentage'],$res['class'],$res['month'],$res['year'],$res['pcissued'],$_SESSION['cert_user']);
...
$pdf->SetMargins(15.59,10,19);
if(($cr=="A" && substr($_POST['htno'],0,2)>="14" ) || ($cr=="D" && substr($_POST['htno'],0,2)>="15") || ($cr=="F" && substr($_POST['htno'],0,2)>="15")){
if(!empty($_POST['image'])){
  $pdf->Ln();
  $pdf->setFont($ffamily2,$fstyle2,$fsize2-4);
  $pdf->Cell(31,6,'Aadhaar No. :');
  $pdf->setFont($ffamily2,$fstyle2,$fsize2);
  if(!empty($_POST['aadhar'])){
 
    $pdf->Cell(120,6,$_POST['aadhar']);
  }
  else{
    $pdf->Cell(120,4,"Not Provided");
  }

  $pdf->Ln();
  $pdf->Image($_POST['image'],21,55,26.4,33);
  $pdf->Ln();
}
}
