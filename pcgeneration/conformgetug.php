...
$val=array();
$cr="A";
 $val=$q->showUgValues($_POST['printdate'],$cr);
  $e=$q->courseName();
 $f=$q->specName();
 $g=$q->deptName();
  if($val['value']>1){
   ...
   $objPHPExcel->getActiveSheet()->SetCellValue('J2', 'Month & Year');
 
    $objPHPExcel->getActiveSheet()->SetCellValue('K2', 'Percentage');
   $objPHPExcel->getActiveSheet()->SetCellValue('L2', 'Class Awarded');
   $objPHPExcel->getActiveSheet()->SetCellValue('M2', 'Pc ISSUED');
  ...
  $objPHPExcel->getActiveSheet()->SetCellValueExplicit('J'.$j, $val['month'][$i] .",".$val['year'][$i],PHPExcel_Cell_DataType::TYPE_STRING);
  
    $objPHPExcel->getActiveSheet()->SetCellValueExplicit('K'.$j, $val['percentage'][$i],PHPExcel_Cell_DataType::TYPE_STRING);
  $objPHPExcel->getActiveSheet()->SetCellValueExplicit('L'.$j, $val['classawd'][$i],PHPExcel_Cell_DataType::TYPE_STRING);
  $objPHPExcel->getActiveSheet()->SetCellValueExplicit('M'.$j, $val['pcissued'][$i],PHPExcel_Cell_DataType::TYPE_STRING);
